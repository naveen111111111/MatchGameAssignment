...
class App extends Component {
  ...
  matchImage = imageUrl => {
    ...
    if (activeImageUrl === imageUrl) {
      const randomNumber = Math.floor(Math.random() * (30 - 0) + 0)

      const randomUrl = imagesList[randomNumber].imageUrl

      this.setState(prevState => ({
        score: prevState.score + 1,
        activeImageUrl: randomUrl,
      }))
    } else {
  ...
  renderGameView = () => {
    const {activeImageUrl, activeTabId} = this.state
    ...
    return (
      <div className="game-container">
        ...
        <ul className="thumbnails-container">
          {filteredThumbnailsList.map(thumbnailObject => (
            <Thumbnails
              key={thumbnailObject.id}
              thumbnailDetails={thumbnailObject}
  ...
  renderScoreCard = score => (
    <ScoreCard restartGame={this.restartGame} score={score} />
  )
  ...
  render() {
    const {isGameRunning, time, score} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="game-view-container">
          {isGameRunning ? this.renderGameView() : this.renderScoreCard(score)}
        </div>
      </div>
    )
  }
